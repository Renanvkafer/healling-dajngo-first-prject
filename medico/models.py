...
class medico_datas(models.Model):
...
class DadosMedico(models.Model):
    ...
    def proxima_data(self):
        proxima_data = medico_datas.objects.filter(user=self.user).filter(data__gt=datetime.now()).filter(agendado=False).order_by('data').first()
        return proxima_data.data if proxima_data else None
